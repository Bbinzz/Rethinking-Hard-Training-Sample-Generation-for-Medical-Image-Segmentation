...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="is add feedback loss in training",
    )
    parser.add_argument(
        "--revision",
        type=str,
...
def feedback_loss(noise_scheduler,x_noisy,model_output ,t,vae,segmentmodel,weight_dtype,mask,MEAN,STD):
        ...
        # xrec_output = segmentmodel.transform(xrec_output)
        # mask =segmentmodel.mask_transform(mask)
        # lateral_map_5, lateral_map_4, lateral_map_3, lateral_map_2 = segmentmodel(xrec_output)
        #     # ---- pranet loss function ----
        # loss5 = structure_loss(lateral_map_5, mask)
        # loss4 = structure_loss(lateral_map_4, mask)
        # loss3 = structure_loss(lateral_map_3, mask)
        # loss2 = structure_loss(lateral_map_2, mask)
        # segmention_loss = loss2 + loss3 + loss4 + loss5
                
        # 适配息肉数据的修改修改像素值
        # 将所有像素值为0的像素替换为1，将像素值为1的替换为150
        mask = mask[:,0,:,:].long()
        mask[(mask > 0 )] = 151
        mask[(mask<= 0)] = 141
        
        # print(mask)
        img = (images_tensor-MEAN) / STD
        # print(img)
        outputs = segmentmodel(pixel_values=img, labels=mask)
        segmention_loss, logits = outputs.loss, outputs.logits

        # convert segmemntionloss to our loss 
        feedback_loss = -segmention_loss
...
def main(args):
    ...
    image_logs = None
    # protptypoe
    if args.feedback == "use":
            segemention_path = "/data1/wzb/hardsamply/segmentor_checkpint/segformer/realdata_100/final_model"
            # segemention_model = PraNet()
            # msg = segemention_model.load_state_dict(torch.load(segemention_path))

            id2label = json.load(open('/home/wzb/workspace/SegFormer-master/zhuimi-id2label.json', "r"))
            id2label = {int(k): v for k, v in id2label.items()}
            label2id = {v: k for k, v in id2label.items()}
            print("Loaded {segemention_path}")
            segemention_model = SegformerForSemanticSegmentation.from_pretrained(segemention_path,
                                                            num_labels=len(id2label),
                                                            id2label=id2label,
                                                            label2id=label2id,
                                                            ignore_mismatched_sizes=True
                                                            )
            segemention_model.to(accelerator.device)
            segemention_model.eval()
            segemention_model.train = disabled_train
            for param in segemention_model.parameters():
                param.requires_grad = False
            MEAN = torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1).to(accelerator.device)  # 通道均值
            STD = torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1).to(accelerator.device)   # 通道标准差
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if args.feedback == "use":
                    ...
                    loss_dis = feedback_loss(noise_scheduler,noisy_latents,model_pred,timesteps,vae,segemention_model,weight_dtype,mask,MEAN,STD)
                    # if loss_dis != 0:
                    #     print("dis_loss != 0")
                    a = 0.01
